...
@test 'Copy all files' {
    ...
    [[ "$(shorten_fixture_path "$output")" = "$(cat <<-EOF
		cp -r --preserve FIXTURE_PATH/old_home/foo.txt FIXTURE_PATH/new_home_empty/foo.txt
...
@test 'Recreate a file/directory in a new home if the same name file already exists' {
    ...
    [[ "$(shorten_fixture_path "$output")" = "$(cat <<-EOF
		rm -rf FIXTURE_PATH/new_home_nonempty/foo.txt
